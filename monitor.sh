...
getGfxInfo(){
dumpsys gfxinfo "$1" reset &
while true;do
  if [ -f /data/local/tmp/stop ];then
    dumpsys gfxinfo "$1" | sed -n '6,19'p >>$monitor/fps_gfxinfo.txt
#    dumpsys gfxinfo "$1" framestats|$bb awk '/^Flags/,/---PROFILEDATA---$/{print $0}' >>$monitor/fps_gfxinfo.csv
    wait
		break
	fi
done
...
GFX $target $KPI $type $delay $frames
